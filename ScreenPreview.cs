...
// arrives, and at every supported panel size. Not part of the service; run with:
...
public static class ScreenPreview {
    public static void Run(string outputDirectory) {
        ...
        foreach (var height in new[] { 64, 32 }) {
            var options = new DisplayOptions { Height = height };
            ...
            foreach (var (name, update) in samples) {
                ...
                var path = Path.Combine(outputDirectory, $"{height}-{name}.png");
